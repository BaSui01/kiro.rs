...
export function SettingsPage({ onBack }: SettingsPageProps) {
  ...
  const handleSaveConfig = async () => {
    ...
    if (
      configForm.proxyUrl &&
      !configForm.proxyUrl.match(/^(https?|socks5):\/\/.+/)
    ) {
      toast.error(
        "代理地址格式不正确，应为 http://、https:// 或 socks5:// 开头"
      );
      return;
    }
  ...
  const handleSavePoolBinding = async () => {
    ...
    try {
      ...
      toast.success(
        editPoolId === "__auto__" ? "已设置为自动路由" : "已绑定到池"
      );
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  {editingConfig ? (
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {config?.sessionCacheTtlSecs} 秒（
                      {Math.round((config?.sessionCacheTtlSecs || 0) / 60)}{" "}
                      分钟）
                    </p>
                  )}
                </div>
                  ...
                  {apiKeys.map((key) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {key.poolId === "__auto__" ? (
                            ...
                            <Badge variant="outline" className="gap-1">
                              ...
                              {pools.find((p) => p.id === key.poolId)?.name ||
                                key.poolId}
                            </Badge>
                          ) : (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Switch
                          checked={key.enabled}
                          onCheckedChange={() => handleToggleApiKey(key)}
                        />
                        <Button
                          variant="ghost"
                          size="icon"
            ...
            <>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select
                    value={newApiKeyPoolId}
                    onValueChange={setNewApiKeyPoolId}
                  >
                    <SelectTrigger>
